...
systick_isr:
    ...
    BL actualizar_reloj
    BL refresco 
    //BL led_prender// para cuando estoy incrementando , lo enciendo cuando llega a 24
...
/************************************************************************************/
/* Rutina funcion tecla S3  - Para volver el contador a 000 y apagar LED                                                        */
/************************************************************************************/
.func tecla_3
tecla_3:
    //cargo 24 segundos
...
actualizar_reloj:
    ...
    BLMI incremento_dseg    //rutina suma de 0,1s
    STRB R3,[R2]
    POP {PC}
.endfunc

/************************************************************************************/
/* Rutina de incrementar 0,1s                                                       */
/************************************************************************************/
.func incremento_dseg
incremento_dseg:
    PUSH {LR}
    PUSH {R2,R3}
    LDR R0,=tiempo          // R0 = direccion de tiempo
    LDRB R1,[R0]            // R1 = valor decima de seg (display1)
    LDR R2,=bandera
    LDRB R2,[R2]            // R2 = bandera
    TST R2, #1              // Comprueba el estado de la bandera
    IT NE                   // Si bandera = 1 (tecla presionada)
    ADDNE R1,#1             // tiempo + 0,1 (incrementa 1 al dígito de décima de seg)
    CMP R1,#10
    ITT GE                  // Si décima de seg >= 10
    MOVGE R1,#0             // Reinicia décima de seg a 0
    BLGE incrementar_seg    // Llama a la rutina para incrementar un segundo
    STRB R1,[R0]            // Guarda el valor actualizado de décima de seg
    POP {R2,R3}
    POP {PC}
.endfunc
/************************************************************************************/
...
/************************************************************************************/
.func incrementar_seg
incrementar_seg:
    PUSH {LR}
    LDR R0,=tiempo          // R0 = dirección de 'tiempo'
    LDRB R2,[R0,#1]         // R2 = unidades de segundos
    LDRB R3,[R0,#2]         // R3 = decenas de segundos

    // Compara con el tope (24 segundos)
    LDR R4,=tope            // R4 = dirección de 'tope'
    LDRB R5,[R4,#1]         // R5 = unidades de tope
    LDRB R6,[R4,#2]         // R6 = decenas de tope

    CMP R3,R6               // Compara decenas de segundos
    IT NE
    BNE skip_increment      // Salta si las decenas no son iguales
    CMP R2,R5               // Compara unidades de segundos
    IT NE
    BNE skip_increment      // Salta si las unidades no son iguales

    // Si los segundos son iguales al tope (24), detener el conteo y encender el LED
    MOV R2,#0               // Reinicia unidades de segundos a 0
    MOV R3,#0               // Reinicia decenas de segundos a 0
    MOV R1,#0         //decima de seg = 0
    BL flag_dis       //Bandera = 0 //dejar de contar
    B fin_incremento        // Salta al final de la rutina

skip_increment:
    ADD R2,#1               // Incrementa unidades de segundos
    CMP R2,#10
    ITT GE                  // Si unidades de segundos >= 10
    MOVGE R2,#0             // Reinicia unidades de segundos a 0
    ADDGE R3,#1             // Incrementa decenas de segundos
    STRB R2,[R0,#1]         // Guarda unidades de segundos
    STRB R3,[R0,#2]         // Guarda decenas de segundos
